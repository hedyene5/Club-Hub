...
export class ClubDetailComponent implements OnInit, OnDestroy {
  ...
  isAdmin = false;        // PRESIDENT / RH / SECRETAIRE_GENERALE — gestion membres
  isPresident = false;    // PRESIDENT seulement — édition/suppression du club
  ...
  ngOnInit(): void {
    ...
    // Edit/Delete the club itself is reserved to the PRESIDENT — RH and the
    // Secretary General can manage members but must not alter the club entity.
    this.isPresident = role === 'PRESIDENT';

    console.log('🔍 ngOnInit - Role:', role, 'isAdmin:', this.isAdmin, 'isPresident:', this.isPresident);
  ...
  addMember(): void {
    ...
    this.authService.createUser(newUser).subscribe({
      ...
      error: (err) => {
        ...
        // Affiche le vrai message du backend pour faciliter le debug.
        // Backend renvoie soit:
        //   - { error: "..." } (notre nouvel endpoint POST /api/users)
        //   - une string brute (ancien comportement /api/auth/register)
        //   - un ProgressEvent (erreur réseau / CORS / serveur down)
