        ...
        <header class="fixed top-0 left-0 right-0 z-50 bg-white border-b border-zinc-100 shadow-sm">
            <div class="max-w-screen-2xl mx-auto px-6 h-16 flex items-center justify-between">
                ...
                <nav class="hidden md:flex items-center gap-1 bg-zinc-50 rounded-3xl p-1 border border-zinc-100">
                    ...
                    <!-- ONLY for bureau roles: show Dashboard -->
                    <ng-container *ngIf="isBureau$ | async">
                        <a routerLink="/app/dashboard" routerLinkActive="active-tab"
                           class="nav-tab px-6 py-2.5 text-sm font-medium rounded-3xl transition-all">
                            Dashboard
                        </a>
                    </ng-container>

                    <!-- Rest of navigation items unchanged -->
                    ...
                    <!-- ... rest of navigation (Operations, Virtual Events, etc.) exactly as before ... -->
                    <div class="relative group">
                        <button class="nav-tab px-6 py-2.5 text-sm font-medium rounded-3xl transition-all flex items-center gap-1">
                ...
                <div class="flex items-center gap-6" *ngIf="currentUser$ | async as currentUser">
...
export class FrontOfficeHeaderComponent {
    ...
    isBureau$: Observable<boolean>;

    constructor(private authService: AuthService) {
        this.currentUser$ = this.authService.userProfile$;
        this.isBureau$ = this.currentUser$.pipe(
            map(user => {
                if (!user) return false;
                const role = (user.role || '').toUpperCase();
                return ['PRESIDENT', 'VICE_PRESIDENT', 'SECRETAIRE_GENERALE', 'TRESORIER', 'RH'].includes(role);
            })
        );
    }
}