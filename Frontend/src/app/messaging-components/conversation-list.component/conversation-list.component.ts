...
export class ConversationListComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    this.conversationSubscriptions.forEach(id => this.webSocketService.unsubscribeFromConversation(id));
  }
  ...
  private listenForNewMessages(): void {
    ...
    this.wsSub = this.webSocketService.message$.subscribe((msg: any) => {
      ...
      console.log('🟢 [LIST] WebSocket message received for conv:', msg.conversationId);
      ...
      this.conversations = [conv, ...this.conversations.filter(c => c.id !== msg.conversationId)];
    });
  }

  // Main fix for leaving
  onConversationLeft(): void {
    this.selectedConversation = null;
    this.loadConversations();   // Always reload full list from backend after leaving
  }
  ...
  // ... rest of your methods (openNewChatModal, closeNewChatModal, getAvatarInitial, filteredConversations, toggleSidebar, onConvImageError)
