...
export class ChatWindowComponent implements OnInit, OnChanges, OnDestroy {
    ...
    availableParticipants: { userId: string; name: string }[] = [];
    transferringConversationId: string | null = null;
    ...
    deleteConversationForMe() {
        ...
        this.conversationService.leaveConversation(this.conversation.id, this.currentUserId)
            .subscribe({
                next: (res: any) => {
                    if (res?.success) {
                        this.messages = [];
                        this.conversationLeft.emit();
                    }
                },
                error: (err) => {
                    if (err?.error?.error === 'TRANSFER_REQUIRED') {
                        this.transferringConversationId = this.conversation!.id;
                        this.loadAvailableParticipants();
                    } else {
                        alert('Action failed. Please try again.');
                        console.error('Leave/archive error', err);
                    }
                }
            });
    }
        ...
        loadAvailableParticipants() {
            ...
            this.conversationService.getParticipants(this.conversation.id).subscribe({
                next: (participants) => {
                    this.availableParticipants = participants
                        .filter(p => p.userId !== this.currentUserId)
                        .map(p => ({
                            userId: p.userId,
                            name: (p as any).userName || p.userId
                        }));
                    this.showTransferModal = true;
                    this.cdr.detectChanges();   // <-- THIS MUST BE HERE
                },
                error: () => alert('Failed to load participants.')
            });
        }

    transferAndLeave(toUserId: string) {
        if (!this.transferringConversationId) return;

        this.conversationService.transferSuperAdmin(
            this.transferringConversationId,
            this.currentUserId,
            toUserId
        ).subscribe({
            next: () => {
                this.showTransferModal = false;
                this.transferringConversationId = null;
                this.deleteConversationForMe();
            },
            error: (err) => {
    ...
    onImageError(event: any) {
        ...
        event.target.src = 'https://via.placeholder.com/150?text=Group';
    }
