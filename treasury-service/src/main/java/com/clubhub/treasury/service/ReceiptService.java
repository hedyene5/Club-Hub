...
public class ReceiptService {
    ...
    public byte[] generatePdfBytes(String paymentId, String memberName, String clubName) {
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            addDetailRow(details, "Statut", "PAYE", false);
            addDetailRow(details, "Date d'echeance",
                    payment.getDueDate() != null ? payment.getDueDate().format(DATE_FR) : "-", true);
            addDetailRow(details, "Date de paiement",
                    payment.getPaidAt() != null ? payment.getPaidAt().format(DATETIME_FR) : "-", false);

            if (payment.getInstallmentNumber() != null && payment.getTotalInstallments() != null) {
                addDetailRow(details, "Echeance",
                        payment.getInstallmentNumber() + " / " + payment.getTotalInstallments(), true);
            }

            if (payment.getStripePaymentIntentId() != null) {
                addDetailRow(details, "Reference transaction", payment.getStripePaymentIntentId(), true);
            }
