...
public class BilanService {
    ...
    public BilanResponse generateBilan(Long clubId, LocalDate start, LocalDate end, String periodLabel) {
        ...
        List<Payment> payments = allPayments.stream()
                .filter(p -> p.getCreatedAt() != null && !p.getCreatedAt().toLocalDate().isBefore(start) && !p.getCreatedAt().toLocalDate().isAfter(end))
                .toList();

        List<Expense> expenses = allExpenses.stream()
                .filter(e -> e.getCreatedAt() != null && !e.getCreatedAt().toLocalDate().isBefore(start) && !e.getCreatedAt().toLocalDate().isAfter(end))
                .toList();
