...
public class ParticipantService {
    ...
    public Map<String, String> leaveConversation(String conversationId, String userId) {
        ...
        if (participant.getRole() == ConversationParticipant.RoleParticipant.SUPERADMIN) {
        ...
        if (convOpt.isEmpty()) {
            result.put("error", "Conversation introuvable");
            return result;
        }

        Conversation conv = convOpt.get();

        if (conv.getType() == Conversation.TypeConversation.GROUP) {
            // === GROUP: Completely remove the participant ===
            participantRepository.delete(participant);
            result.put("success", "true");
            result.put("action", "removed_from_group");
            System.out.println("User " + userId + " removed from GROUP " + conversationId);
        } else {
            // === PRIVATE: Only hide messages (archive for me) ===
            participant.setMessagesHidden(true);

            participantRepository.save(participant);
            result.put("success", "true");
            result.put("action", "archived_private");
            System.out.println("Private conversation " + conversationId + " archived for user " + userId);
        }
