...
export class InstantVoiceComponent implements OnInit, OnDestroy {
  ...
  submitReport() {
    ...
    this.http.post('http://localhost:8084/api/reports', {
  ...
  openChannel(channel: Channel) {
    ...
    this.http.get<AppUser[]>(`${this.GATEWAY_URL}/api/users`).subscribe({
      next: (users) => {
  ...
  loadAudioHistory(channelId: string) {
    ...
    this.http.get<AudioMessage[]>(`http://localhost:8084/api/channels/${channelId}/audio`).subscribe({
    ...
    return new Promise((resolve) => {
      ...
      reader.onloadend = () => {
        ...
        this.http.post<AudioMessage>(
          `http://localhost:8084/api/channels/${this.selectedChannel!.id}/audio`,
