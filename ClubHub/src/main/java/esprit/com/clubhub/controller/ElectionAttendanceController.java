...
public class ElectionAttendanceController {
     ...
     * 
     * POST /api/elections/{electionId}/attendance/scan
     * Body: { "qrData": "...", "scannedBy": "userId" }
     */
    @PostMapping("/scan")
     ...
     * 
     * GET /api/elections/{electionId}/attendance/list
     */
    @GetMapping("/list")
    public ResponseEntity<List<ElectionAttendance>> getAttendanceList(@PathVariable String electionId) {
        List<ElectionAttendance> attendanceList = attendanceService.getAttendanceList(electionId);
     ...
     * 
     * GET /api/elections/{electionId}/attendance/stats
     */
    @GetMapping("/stats")
    public ResponseEntity<Map<String, Object>> getAttendanceStats(@PathVariable String electionId) {
        Map<String, Object> stats = attendanceService.getAttendanceStats(electionId);
        return ResponseEntity.ok(stats);
    }

    /**
     * Vérifier si un utilisateur peut scanner les QR codes
     * 
     * GET /api/elections/{electionId}/attendance/can-scan?userId={userId}&clubId={clubId}
     */
    @GetMapping("/can-scan")
    public ResponseEntity<Map<String, Boolean>> canScanQRCodes(
        ...
        boolean canScan = attendanceService.canScanQRCodes(clubId, userId);
        return ResponseEntity.ok(Map.of("canScan", canScan));
    }

    /**
     * Valider un token de vote
     * 
     * GET /api/elections/{electionId}/attendance/validate-token?token={token}
     */
