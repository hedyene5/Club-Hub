...
public class VirtualEventController {
    ...
    private final EmailService vemEmailService;
    ...
    public VirtualEventController(
            ...
            EmailService vemEmailService
    ) {
        this.virtualEventService = virtualEventService;
        this.registrationService = registrationService;
        this.service = service;
        this.pdfService = pdfService;
        this.vemEmailService = vemEmailService;
    }

    /**
     * Même contenu que {@code POST /api/email/test-confirmation} — chemin ici pour que le
     * front (Gateway → /api/virtual-events/**) n’ait pas de collision avec
     * {@code GET /api/virtual-events/{id}} pour un segment "confirmation-email".
     */
    @PostMapping("/confirmation-email")
    public ResponseEntity<String> sendConfirmationEmail(@RequestBody Map<String, String> body) {
        vemEmailService.sendRegistrationConfirmation(
                body.get("to"),
                body.get("userName"),
                body.get("eventTitle"),
                body.get("eventDate"),
