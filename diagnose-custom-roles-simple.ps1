...
try {
    $response = Invoke-WebRequest -Uri "$apiUrl/clubs" -UseBasicParsing -TimeoutSec 5 -ErrorAction Stop
...
try {
    $clubsResponse = Invoke-WebRequest -Uri "$apiUrl/clubs" -UseBasicParsing -TimeoutSec 5 -ErrorAction Stop
...
try {
    $permsResponse = Invoke-WebRequest -Uri "$apiUrl/roles/permissions" -UseBasicParsing -TimeoutSec 5 -ErrorAction Stop
    ...
    Write-Host "   ❌ API permissions non accessible: $_" -ForegroundColor Red
}
...
try {
    $rolesResponse = Invoke-WebRequest -Uri "$apiUrl/roles/club/$clubId" -UseBasicParsing -TimeoutSec 5 -ErrorAction Stop
    ...
    if ($customRoles.Count -eq 0) {
        ...
        Write-Host ""
        Write-Host "   Pour créer un rôle de test automatiquement:" -ForegroundColor Yellow
        ...
        try {
            ...
            # Recharger les rôles
            $rolesResponse = Invoke-WebRequest -Uri "$apiUrl/roles/club/$clubId" -UseBasicParsing -ErrorAction Stop
            $customRoles = ($rolesResponse.Content | ConvertFrom-Json)
        } catch {
...
Write-Host "3. Cliquer sur le bouton 🐛 (debug)" -ForegroundColor White
Write-Host "4. Vérifier la console du navigateur (F12)" -ForegroundColor White
Write-Host "5. Chercher les logs de chargement des rôles" -ForegroundColor White
Write-Host ""
