 ...
 *
 * Matrice de référence (Events comme exemple) :
 *   - Bureau exécutif       : Members + Roles + Notifications + Profile (+ Settings si Pres/VP)
 *   - Trésorier             : (Bureau) + approuver/gérer les devis (page "Devis")
 *   - Responsable Events    : Calendar + All events + QR Scanner + Assign Tasks
 *                             + Event Needs + Lenders  (PAS Manage Tasks ni Devis)
 *   - Membre du comité Events : Calendar + RSVP + Manage Tasks
 *   - Membre simple hors comité : Calendar + RSVP
 */
type SidebarPermissions = {
  ...
  canUseQrScanner: boolean;          // QR scanner (check-in à l'entrée)
  canAssignTasks: boolean;           // créer/distribuer des tâches
  canViewMyTasks: boolean;           // voir & màj ses tâches assignées
  canManageBorrowedItems: boolean;   // matériel/besoins pour l'événement
...
export class AppSidebarComponent implements OnInit, OnDestroy {
  ...
  private baseNavItems: NavItem[] = [
    ...
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 6.44V9.77M12.02 2C8.34 2 5.36 4.98 5.36 8.66V10.76C5.36 11.44 5.08 12.46 4.73 13.04L3.46 15.16C2.68 16.47 3.2 17.93 4.63 18.41C9.07 19.91 14.97 19.91 19.41 18.41C20.74 17.97 21.3 16.38 20.57 15.16L19.3 13.04C18.95 12.46 18.67 11.43 18.67 10.76V8.66C18.66 5 15.66 2 12.02 2Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><path d="M15.33 18.82C15.33 20.65 13.83 22.15 12 22.15C11.09 22.15 10.25 21.77 9.65 21.17C9.05 20.57 8.67 19.73 8.67 18.82" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>`,
      name: 'Notifications',
      path: '/notifications',
    },
    {
  ...
  othersItems: NavItem[] = [
    ...
    {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 15C13.6569 15 15 13.6569 15 12C15 10.3431 13.6569 10 12 10C10.3431 10 10 10.3431 10 12C10 13.1046 10.8954 15 12 15Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M2 12.88V11.12C2 10.08 2.85 9.22 3.9 9.22C5.71 9.22 6.45 7.94 5.54 6.37C5.02 5.47 5.33 4.3 6.24 3.78L7.97 2.79C8.76 2.32 9.78 2.6 10.25 3.39L10.36 3.58C11.26 5.15 12.74 5.15 13.65 3.58L13.76 3.39C14.23 2.6 15.25 2.32 16.04 2.79L17.77 3.78C18.68 4.3 18.99 5.47 18.47 6.37C17.56 7.94 18.3 9.22 20.11 9.22C21.15 9.22 22.01 10.07 22.01 11.12V12.88C22.01 13.92 21.16 14.78 20.11 14.78C18.3 14.78 17.56 16.06 18.47 17.63C18.99 18.54 18.68 19.7 17.77 20.22L16.04 21.21C15.25 21.68 14.23 21.4 13.76 20.61L13.65 20.42C12.75 18.85 11.27 18.85 10.36 20.42L10.25 20.61C9.78 21.4 8.76 21.68 7.97 21.21L6.24 20.22C5.33 19.7 5.02 18.53 5.54 17.63C6.45 16.06 5.71 14.78 3.9 14.78C2.85 14.78 2 13.92 2 12.88Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
      name: 'Club Settings',
      path: '/club/settings',
    },
  ];
  ...
  private get permissions(): SidebarPermissions {
    ...
    const isSimpleMember = role === 'MEMBRE_SIMPLE';
    ...
    // Membre "ordinaire" du club (pas dans le bureau)
    const isOrdinaryMember = isSimpleMember || (!isBureau && !isRespEvents);

    return {
      // Calendar : seulement les acteurs du comité Events (responsable + membres)
      // Un membre simple HORS comité ne voit QUE le bouton RSVP.
      canViewCalendar:        isRespEvents || isMemberOfEventCommittee,

      // All events (page CRUD admin) : responsable Events seulement
      canManageEvents:        isRespEvents,

      // RSVP : membres ordinaires (simple ou dans le comité), PAS le responsable
      canRsvpEvents:          !isRespEvents && (isOrdinaryMember || isMemberOfEventCommittee),

      // QR Scanner : responsable Events seulement (check-in à la porte)
      canUseQrScanner:        isRespEvents,

      // Assign Tasks (créer/distribuer) : responsable Events seulement
      canAssignTasks:         isRespEvents,

      // Manage Tasks (voir / màj mes tâches) : MEMBRES SIMPLES du comité Events
      // uniquement (PAS le responsable — lui assigne, il ne "manage" pas).
      canViewMyTasks:         isMemberOfEventCommittee && !isRespEvents,

      // Event Needs + Lenders : responsable Events seulement
      canManageBorrowedItems: isRespEvents,
      canManageLenders:       isRespEvents,

      // Devis (quotes) : Trésorier uniquement (ni responsable, ni event manager)
      canManageDevis:         isTreasurer,

      canManageMembers:       isBureau,
      canManageRoles:         isBureau,
      canManageClubSettings:  isPresOrVP,
    };
  }

  /**
   * Construit dynamiquement le sidebar :
   * 1) Insère "Mon Club" après le Dashboard
   * 2) Filtre items + subItems selon les permissions
   */
  get navItems(): NavItem[] {
    ...
    const monClubItem: NavItem = {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 4c1.1 0 2 .9 2 2s-.9 2-2 2-2-.9-2-2 .9-2 2-2zm0 13c-2.33 0-4.31-1.46-5.11-3.5h10.22c-.8 2.04-2.78 3.5-5.11 3.5z" fill="currentColor"/></svg>`,
      name: clubName,
      path: clubId ? `/clubs/${clubId}` : '/clubs'
    };

    // On part toujours du Dashboard + Mon Club
    const items: NavItem[] = [this.baseNavItems[0], monClubItem];

    for (const item of this.baseNavItems.slice(1)) {
    ...
    switch (item.name) {
      case 'Events': {
        const subItems = (item.subItems ?? []).filter(sub => {
          switch (sub.name) {
            case 'Calendar':    return p.canViewCalendar;
            case 'All events':  return p.canManageEvents;
            case 'RSVP':        return p.canRsvpEvents;
            case 'QR Scanner':  return p.canUseQrScanner;
      ...
      case 'Event Needs': {
        const subItems = (item.subItems ?? []).filter(sub => {
          switch (sub.name) {
            case 'All needs':       return p.canManageBorrowedItems;
  ...
  /** "Others" filtré : Profile toujours visible, Club Settings réservé Pres/VP. */
  get visibleOthersItems(): NavItem[] {
    const p = this.permissions;
