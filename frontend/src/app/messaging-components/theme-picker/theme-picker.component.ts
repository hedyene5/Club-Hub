...
import { AuthService } from '../../services/User/auth.service'; // ← add
...
export class ThemePickerComponent implements OnInit {
  ...
  private loadPresets() {
    this.http.get<Theme[]>('http://localhost:8081/api/conversations/themes/presets')
        .subscribe({
          next: (data) => {
            this.presets = data;
  ...
  generateWithAI() {
    ...
    this.http.post<Theme>(
        `http://localhost:8081/api/conversations/${this.conversationId}/theme/generate`,
        this.aiPrompt,
        {
          headers: {
  ...
  private applyTheme(theme: Theme) {
    this.http.put<Theme>(
        `http://localhost:8081/api/conversations/${this.conversationId}/theme`,
        theme,
        {
