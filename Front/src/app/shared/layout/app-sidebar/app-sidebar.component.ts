...
export class AppSidebarComponent {
  ...
  // Base nav items (sans le lien vers "Mon Club" car il est dynamique)
  ...
  // ✅ GETTER pour les navItems dynamiques (avec "Mon Club")
  get navItems(): NavItem[] {
    const clubId = this.currentUser?.clubId;
    
    // Item "Mon Club" (dynamique)
    const monClubItem: NavItem = {
      icon: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 4c1.1 0 2 .9 2 2s-.9 2-2 2-2-.9-2-2 .9-2 2-2zm0 13c-2.33 0-4.31-1.46-5.11-3.5h10.22c-.8 2.04-2.78 3.5-5.11 3.5z" fill="currentColor"/>
             </svg>`,
      name: "Mon Club",
      path: clubId ? `/clubs/${clubId}` : '/clubs'
    };
    
    // Insérer "Mon Club" après Dashboard (index 1)
    return [this.baseNavItems[0], monClubItem, ...this.baseNavItems.slice(1)];
  }
