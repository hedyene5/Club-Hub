...
export class ProfileComponent implements OnInit {
  ...
  saveChanges(): void {
    ...
    this.http.put(`http://localhost:8084/api/users/${this.user.userId}`, updateData).subscribe({
      next: (response: any) => {
  ...
  private updateMemberInClub(): void {
    ...
    this.http.put(
      `http://localhost:8084/api/clubs/${this.user.clubId}/members/${this.user.userId}`,
      memberData
    ).subscribe({
      next: () => {
